# Awesome AI-driven plant biology

This repository provides a curated collection of research and review papers on **AI-based methods in plant science**. It will be continuously updated to track advances at the frontier of AI-driven plant biology. Feel free to follow and star the project.

## Background

Plant science is entering an era of data-driven discovery — from phenomenology to programmability. Although we recently published a review on deep learning in plant research, the field is evolving too rapidly for any static article to remain comprehensive. New architectures, foundation models, and cross-disciplinary advances are emerging at an unprecedented pace. To address this, we created this repository as a living, community-driven knowledge hub for AI in plant biology. It continuously curates and organizes machine learning advances relevant to plant research, including methods developed beyond the plant domain but adaptable to it.

To the best of our knowledge, this is the first open and systematically organized collection of AI research dedicated to plant science. Papers are categorized by biological application and model type to enable structured exploration and interdisciplinary connection. Our vision is to provide an evolving resource that helps researchers navigate, compare, and apply AI approaches in plant biology, fostering transparency, accessibility, and a more connected research ecosystem.

We welcome contributions from the community. If you would like to suggest new papers or improve the taxonomy, please open an issue or submit a pull request. Together, we can build an up-to-date resource that evolves alongside the field.

## Table of Contents

## Review
- [General](#general)
- [Genomics](#genomics)
- [single-cell omics](#single-cell-omics)
- [Gene expression](#gene-expression)
- [CRE design](#CRE-design)

### Genomic language models
- [gLM](#genomic-language-models)
- [Other](#other-species-gLM)

### Epigenomic models
- [Supervised](#supervised-plant)
- [Other](#other-species-epLM)

### Single-cell omics
- [Single-cell omics](#single-cell-omics)

### Expression
- [Plant](#plant-gene-expression)
- [Other](#other-species-expression)
### Protein & Functional Genomics
- [Gene Function Prediction / De Novo Gene Discovery](#gene-function-prediction--de-novo-gene-discovery)
- [CRE engineering/design](#CRE-engineering-and-design)
- [Protein Engineering/Design](#protein-engineering-and-design)

### Evolution
- [Phylogenetics](#phylogenetics)

### Resources
- [Dataset](#dataset)

### Others
- [Other](#others)

## Review

### general

**Large language models in plant biology.**

Lam, H. Y. I., Ong, X. E., & Mutwil, M. (2024). *Trends in plant science*, *29*(10), 1145–1155. https://doi.org/10.1016/j.tplants.2024.04.013

**Application of machine learning and genomics for orphan crop improvement.**

MacNish, T.R., Danilevicz, M.F., Bayer, P.E. *et al.* *Nat Commun* **16**, 982 (2025). https://doi.org/10.1038/s41467-025-56330-x

**Integrated biotechnological and AI innovations for crop improvement.** 

Li, G., An, L., Yang, W. *et al.* *Nature* **643**, 925–937 (2025). https://doi.org/10.1038/s41586-025-09122-8

- Broadly discuss the future of AI and argobiotech, covering multiomics, genome editing, protein design, AI-guided evolution/engieering, plant phenomics, etc.

**Programmable genome engineering and gene modifications for plant biodesign.**

Liu, J., Zhang, R., Chai, N., Su, L., Zheng, Z., Liu, T., Guo, Z., Ma, Y., Xie, Y., Xie, X., Lin, Q., Chen, L., Liu, Y. G., & Zhu, Q. (2025). *Plant communications*, *6*(8), 101427. https://doi.org/10.1016/j.xplc.2025.101427

- For genome editing and synthetic biology, this is the go-to review.

**A comprehensive survey of genome language models in bioinformatics.**

Liyuan Shu, Jiao Tang, Xiaoyu Guan, Daoqiang Zhang, *Briefings in Bioinformatics*, 27 (1), bbaf724. Jan 2026. https://doi.org/10.1093/bib/bbaf724

### genomics

**Wang H, Chen M, Wei X, et al. Computational tools for plant genomics and breeding.** 

*Sci China Life Sci*. 2024;67(8):1579-1590. https://doi.org/10.1007/s11427-024-2578-6

**The DNA dialect: a comprehensive guide to pretrained genomic language models.** 

Veiner, M., Supek, F. *Mol Syst Biol* **22**, 309–332 (2026). https://doi.org/10.1038/s44320-025-00184-4

**Transformers and genome language models.** 

Consens, M.E., Dufault, C., Wainberg, M. *et al.* *Nat Mach Intell* **7**, 346–362 (2025). https://doi.org/10.1038/s42256-025-01007-9

**The gene function prediction challenge: Large language models and knowledge graphs to the rescue.** 

Sunil, R. S., Lim, S. C., Itharajula, M., & Mutwil, M. (2024). *Current opinion in plant biology*, *82*, 102665. https://doi.org/10.1016/j.pbi.2024.102665

**Identification, characterization, and design of plant genome sequences using deep learning.**

Zhenye Wang, Hao Yuan, Jianbing Yan, Jianxiao Liu. *the Plant Journal*. December 2024 https://doi.org/10.1111/tpj.17190

**Genomic language models: opportunities and challenges.**

Benegas, G., Ye, C., Albors, C., Li, J. C., & Song, Y. S. (2025). *Trends in genetics*, 41(4), 286–302. https://doi.org/10.1016/j.tig.2024.11.013

### single-cell omics

**Advancing plant single-cell genomics with foundation models.**

Multimodal foundation transformer models for multiscale genomics.

Khan, S.A., Martínez-de-Morentin, X., Alsabbagh, A.R. *et al.* *Nat Methods* **23**, 299–311 (2026). https://doi.org/10.1038/s41592-025-02918-6

**Interpretation, extrapolation and perturbation of single cells.** 

Dimitrov, D., Schrod, S., Rohbeck, M. *et al.* *Nat Rev Genet* (2026). https://doi.org/10.1038/s41576-025-00920-4

Chau TN, Wang X, McDowell JM, Li S. *Curr Opin Plant Biol*. 2024;82:102666. https://doi.org/10.1016/j.pbi.2024.102666

### gene expression

**Machine learning methods to model  multicellular complexity and tissue specificity.**

Sealfon, R.S.G., Wong, A.K. & Troyanskaya, O.G. *Nat Rev Mater* **6**, 717–729 (2021). https://doi.org/10.1038/s41578-021-00339-3

- This is excellent paper timely reviewed the exciting moment brought by ML/DL to model multicllular complexitiy of gene expression, particularly tissue-specific and cell-type-specific expression, and provides an outlook for future studies. As it published on 2021, the reviewed methods are not novel, albeit it can bring a lot of ideas for current studies when reading it.

### CRE design

**Modelling and design of transcriptional enhancers.**

De Winter, S., Konstantakos, V. & Aerts, S. *Nat Rev Bioeng* **3**, 374–389 (2025). https://doi.org/10.1038/s44222-025-00280-y

**Hold out the genome: a roadmap to solving the *cis*-regulatory code.**

de Boer, C.G., Taipale, J. *Nature* **625**, 41–50 (2024). https://doi.org/10.1038/s41586-023-06661-w

**Rewiring gene circuitry for plant improvement.**

Borowsky, A.T., Bailey-Serres, J. *Nat Genet* **56**, 1574–1582 (2024). https://doi.org/10.1038/s41588-024-01806-7

**Toward learning the principles of plant gene regulation.**

Jan Zrimec, Aleksej Zelezniak, Kristina Gruden. Trends in Plant Science *27*: 1206-1208 (2022). https://doi.org/10.1016/j.tplants.2022.08.010

**Cis-regulatory sequences in plants: Their importance, discovery, and future challenges.**

Robert J Schmitz, Erich Grotewold, Maike Stam. *The Plant Cell*, 34 (2): 718–741, 2022. https://doi.org/10.1093/plcell/koab281

## genomic language models
### Plant
**BOTANIC-0: a series of foundation models for plant genomic data.**

Jean Ogier du Terrail, Tanguy Marchand, Vincent Cabeli, Zhor Khadir, Cyril Véran, Léonard Strouk. *bioRxiv*, 2026. https://doi.org/10.64898/2026.02.23.706817

**PlantCAD2: A Long-Context DNA Language Model for Cross-Species Functional Annotation in Angiosperms.**

Zhai J, Gokaslan A, Hsu SK, et al.  *bioRxiv*, Dec, 2025. https://doi.org/10.1101/2025.08.27.672609

- a new version of PlantCaduceus (PlantCAD). This new version trained on 65 plant genomes and increased context DNA length from 512 to 8192bp. Notably, the model is reverse-complement equivariant.
- Fine-tuning this model on downstream tasks like accessibility, gene expression, TFBS prediction also demonstrated SOTA. The performance surpassed ArgoNT, GPN (trained on A.th.) and a general model Evo2 (7B).
- Genuinely, there are not many plant gLMs. I wonder whether the zero-shot performance of PlantCAD2 is better than task-specific foundation models like sequence-to-accessibility models or sequence-to-expression models. Cell-type-specificity and tissue specificity of epigenetic marks and expression are also modeled.
- I would love to see whether the PlantCAD2 can be applied to the CRE design or in silico evolution of gene function/expresion.

**Cross-species modeling of plant genomes at single-nucleotide resolution using a pretrained DNA language model.**

Zhai, J., Gokaslan, A., Schiff, Y., Berthel, A., Liu, Z. Y., Lai, W. Y., Miller, Z. R., Scheben, A., Stitzer, M. C., Romay, M. C., Buckler, E. S., & Kuleshov, V. *Proc. Natl. Acad. Sci. U.S.A.* 122 (24) e2421738122 (2025). https://doi.org/10.1073/pnas.2421738122

**PDLLMs: A group of tailored DNA large language models for analyzing plant genomes.**

Liu, G., Chen, L., Wu, Y., Han, Y., Bao, Y., & Zhang, T. (2025). *Molecular plant*, *18*(2), 175–178. https://doi.org/10.1016/j.molp.2024.12.006

**PlantBiMoE: A Bidirectional Foundation Model with SparseMoE for Plant Genomes.**

Kepeng Lin, Qizhe Zhang, Rui Wang Xuehai Hu, Wei Xu. arXiv 2025. https://arxiv.org/abs/2512.07113

**A foundational large language model for edible plant genomes.**

Mendoza-Revilla, J., Trop, E., Gonzalez, L. *et al.*  *Commun Biol* **7**, 835 (2024). https://doi.org/10.1038/s42003-024-06465-2

**Current genomic deep learning architectures generalize across grass species but not alleles.**

Travis Wrightsman, et al., bioRxiv, Apr, 2024. https://doi.org/10.1101/2024.04.11.589024

**DNA language models are powerful predictors of genome-wide variant effects.**

G. Benegas, S.S. Batra, & Y.S. Song Proc. Natl. Acad. Sci. U.S.A. 120 (44) e2311219120, https://doi.org/10.1073/pnas.2311219120 (2023).

- The first plant-specific genomic language model
- Genomic Pre-trained Network (GPN) is designed to learn genome-wide variant effects through unsupervised pretraining on genomic DNA sequences by training on unaligned reference genomes of ***Arabidopsis thaliana* and seven related species within the Brassicales order**.

### other species gLM

**Genome modelling and design across all domains of life with Evo 2.** 

Brixi, G., Durrant, M.G., Ku, J. *et al.* *Nature* (2026). https://doi.org/10.1038/s41586-026-10176-5

**Advancing regulatory variant effect prediction with AlphaGenome.**

Avsec, Ž., Latysheva, N., Cheng, J. *et al.* *Nature* **649**, 1206–1218 (2026). https://doi.org/10.1038/s41586-025-10014-0

**Nucleotide Transformer: building and evaluating robust foundation models for human genomics.** 

Dalla-Torre, H., Gonzalez, L., Mendoza-Revilla, J. *et al.* *Nat Methods* **22**, 287–297 (2025). https://doi.org/10.1038/s41592-024-02523-z

- the first generation of NT

**Genome modeling and design across all domains of life with Evo 2.**

Garyk Brixi, Patrick D. Hsu, Brian L. Hieet al., *bioRxiv*. Feb, 2025. https://doi.org/10.1101/2025.02.18.638918

**A foundational model for joint sequence-function multi-species modeling at scale for long-range genomic prediction.**

Sam Boshar, Benjamin Evans, Ziqi Tang, et al., *bioRxiv*. Dec, 2025. https://doi.org/10.64898/2025.12.22.695963

- Nucleotide Transformer v3 (NTv3)

## epigenomic models (egLM)

### supervised-plant

**Identification of DNA N6-methyladenine modifications in the rice genome with a fine-tuned large language model.**

Zhang Y, Chen H, Xiang S and Lv Z (2025)  *Front. Plant Sci.* 16:1626539. https://doi.org/10.3389/fpls.2025.1626539



**Cross-species prediction of histone modifications in plants via deep learning.**
Lv T, Han Q, Li Y, Liang C, Ruan Z, Chao H, Chen M, Chen D.
*Genome Biol* **27**, 20 (2026). https://doi.org/10.1186/s13059-025-03929-4

- **PlantSei** is adapted from Sei deep learning framewrok—a multi-task, sequence-based architecture originally designed to predict thousands of regulatory features directly from DNA sequences

**Deep learning on chromatin profiles reveals the cis-regulatory sequence code of the rice genome.** 

Zhou X, Ruan Z, Zhang C, Kaufmann K, Chen D. 

*J Genet Genomics* **52**(6):848-851 (2025). [https://doi.org/10.1016/j.jgg.2024.12.007]

- **Osei** is based on the Sei framework to predict diverse chromatin profiles in rice.

**Comprehensive mapping and modelling of the rice regulome landscape unveils the regulatory architecture underlying complex traits.** 

Zhu, T., Xia, C., Yu, R. *et al.* *Nat Commun* **15**, 6562 (2024). https://doi.org/10.1038/s41467-024-50787-y

- This study first presents a comprehensive regulome atlas for rice (*Oryza sativa*), charting the chromatin accessibility across 23 distinct tissues from three representative varieties. Then they train a deep learning model to model tissue-specific chromatin accessibility in a cross-variety manner.

**Constraint of accessible chromatins maps regulatory loci involved in maize speciation and domestication.** 

Liu, Y., Gao, X., Liu, H. *et al.*  *Nat Commun* **16**, 2477 (2025). https://doi.org/10.1038/s41467-025-57932-1

- Mazie tissue-specific chromatin profiles.



### other species (egLM)

**A sequence-based global map of regulatory activity for deciphering human genetics.**

Chen KM, Wong AK, Troyanskaya OG, Zhou J.

*Nat Genet* **54**, 940–949 (2022). [https://doi.org/10.1038/s41588-022-01102-2]

- **Sei** framework uses a compendium covering 21,907 publicly available *cis*-regulatory profiles and the whole-genome sequence to create a mapping from any sequence to a comprehensive set of sequence classes and predict 21,907 chromatin profiles across >1,300 cell lines and tissues. 

**Learning interpretable representation for context-specific transcription regulatory networks using a foundation model.**

Zhaowei Yu, Dongxu Yang, Qianqian Chen, Yuxuan Zhang, Zhanhao Li, Yucheng Wang, Chenfei Wang, Yong Zhang. *Cell Genomics*,  Jan 2026. https://doi.org/10.1016/j.xgen.2025.101130

## single-cell omics

**scPlantLLM: A Foundation Model for Exploring Single-cell Expression Atlases in Plants.** 

Cao, G., (曹广硕), Chao, H., (晁好瑜), Zheng, W., Lan, Y., Lu, K., Wang, Y., Chen, M., (陈铭), Zhang, H., (张和), & Chen, D., (陈迪俊). *Genomics Proteomics Bioinformatics*. 2025;23(3):qzaf024. https://doi.org/10.1093/gpbjnl/qzaf024



**A single-cell rice atlas integrates multi-species data to reveal *cis*-regulatory evolution.**  

Yan, H., Mendieta, J.P., Zhang, X. *et al.* *Nat. Plants* **11**, 2050–2071 (2025). https://doi.org/10.1038/s41477-025-02106-6

-  a comprehensive epigenomic resource for the rice in single-cell resolution covering 126 cell states across nine organs.

## gene expression prediction
### Plant gene expression

**DeepWheat: predicting the effects  of genomic variants on gene expression  and regulatory activities across tissues  and varieties in wheat using deep learning.**

Ma, Z., Zhang, J., Pei, H. *et al.* *Genome Biol* **26**, 321 (2025). [https://doi.org/10.1186/s13059-025-03809-x]

- DeepWheat is a tissue-specific gene expression predictor integrating epigenomic features, which can be used to targeted *cis*-regulatory elements editing and offering a powerful tool for crop functional genomics and breeding.

**Precise engineering of gene expression by editing plasticity.** 

Qiu Y, Liu L, Yan J, Xiang X, Wang S, Luo Y, et al. *Genome Biol* **26**, 51 (2025). [https://doi.org/10.1186/s13059-025-03516-7]

- Basenji2 is a sequence-to-expression deep learning modelswith long (120K-bp) and short (3K-bp) inputs of genomic sequence in the four plant model species of maize, rice, tomato, and *Arabidopsis thaliana*, and combined the interpretability methods to accurately identify distal and proximal CREs in a genome-wide manner.
- This study introduces a novel idea called "editing plasticity (EP)" to conceptually measure the potential for expression changes due to promoter editing.
- Note the model predictions are experimentally demonstrated in the promoter editing task (*ZmVTE4*) with elevated gene expression and phenotypes *in vivo*.

**Unraveling the cis-regulatory code controlling abscisic acid-dependent gene expression in Arabidopsis using deep learning.**

Helder Opdebeeck,  ProfileDajo Smet,  Sander Thierens,  et al., *bioRxiv*, 2025. https://doi.org/10.1101/2025.06.03.657584

**A scalable method for modulating plant gene expression using a multispecies genomic model and protoplast-based massively parallel reporter assay.**

Sania Jevtic, Colleen Drapek, Christophe Gaillochet, Andrew Brockman, James Cadman, Timo Flesch, Nicolas Kral. *bioRxiv*, Dec 2024. https://doi.org/10.1101/2024.12.05.626999

- A predictive machine learning model underpinning the platform has been trained on a wide range of tissue specific transcriptomic and epigenomic coverage datasets from DNA sequence of 12 plant species, showing competitive performance on RNA-seq coverage prediction across all species. The platform further combines *in silico* DNA sequence mutagenesis and a protoplast-based massively parallel reporter assay (MPRA) to evolve the proximal promoter of the tomato gene SlbHLH96, successfully upregulating the gene expression.

**Modeling 0.6 million genes for the rational design of functional *cis*-regulatory variants and *de novo* design of *cis*-regulatory sequences.** 

Li T, Xu H, Teng S, Suo M, Bahitwa R, Xu M, et al. *Proc Natl Acad Sci U S A*. 2024;121:e2319811121. https://doi.org/10.1073/pnas.2319811121

- PhytoExpr is a deep learning framework capable of predicting both mRNA abundance and plant species using the proximal regulatory sequence as the sole input, trained over 17 representative plant species.

**Deep learning the *cis*-regulatory code for gene expression in selected model plants.**

Levy, B., Xu, Z., Zhao, L., Kremling, K., Altman, R., Wong, P. et al. (2022) FloraBERT: cross-species transfer learning with attention-based neural networks for gene expression prediction. Available from: https://doi.org/10.21203/rs.3.rs-1927200/v1

Peleke, F.F., Zumkeller, S.M., Gültas, M., Schmitt, A. & Szymański, J. *Nature Communications*, 15, 3488 (2024). https://doi.org/10.1038/s41467-024-47744-0

**DeepCBA: a deep learning framework for gene expression prediction in maize based on DNA sequence and chromatin interaction.**

Wang, Z., Peng, Y., Li, J., Li, J., Yuan, H., Yang, S. et al. *Plant Communications*, 5 (2024).  https://doi.org/10.1016/j.xplc.2024.100985

**Genome-wide *cis*-decoding for expression design in tomato using cistrome data and explainable deep learning.**

Akagi, T., Masuda, K., Kuwada, E., Takeshita, K., Kawakatsu, T., Ariizumi, T., Kubo, Y., Ushijima, K., & Uchida, S. (2022). *The Plant cell*, *34*(6), 2174–2187. https://doi.org/10.1093/plcell/koac079

**Evolutionarily informed deep learning methods for predicting relative transcript abundance from DNA sequence.**

J.D. Washburn,M.K. Mejia-Guerra,G. Ramstein,K.A. Kremling,R. Valluru,E.S. Buckler, & H. Wang,  P*roc. Natl. Acad. Sci. U.S.A.* 116 (12) 5542-5549  (2019). https://doi.org/10.1073/pnas.1814551116.

### other species gene expression

**Predicting RNA-seq coverage from DNA sequence as a unifying model of gene regulation.**

Linder, J., Srivastava, D., Yuan, H. *et al.*  *Nat Genet* **57**, 949–961 (2025). https://doi.org/10.1038/s41588-024-02053-6

**A foundation model of transcription across human cell types.** 

Fu, X., Mo, S., Buendia, A. *et al.* *Nature* **637**, 965–973 (2025). https://doi.org/10.1038/s41586-024-08391-z

- GET (general expression transformer) is an interpretable foundation model designed to uncover regulatory grammars across 213 human fetal and adult cell types.

**Learning the histone codes with large genomic windows and three-dimensional chromatin interactions using transformer.** 

Lee, D., Yang, J. & Kim, S. *Nature Communications*, 13, 6678 (2022). https://doi.org/10.1038/s41467-022-34152-5

**Effective gene expression prediction from sequence by integrating long-range interactions.**

Avsec, Ž., Agarwal, V., Visentin, D. *et al.* *Nat Methods* **18**, 1196–1203 (2021). https://doi.org/10.1038/s41592-021-01252-x

- Enformer

**Deep learning sequence-based ab initio prediction of variant effects on expression and disease risk.**

Zhou, J., Theesfeld, C.L., Yao, K., Chen, K.M., Wong, A.K. & Troyanskaya, O.G. *Nature Genetics*, 50, 1171–1179 (2018). https://doi.org/10.1038/s41588-018-0160-6

- Expecto

## gene function prediction / de novo gene discovery

**Harnessing DNA Foundation Models for Cross-Species Transcription Factor Binding Site Prediction in Plant Genomes.**

Maryam Haghani,  ProfileKrishna Vamsi Dhulipalla, Song Li. *bioRxiv*, 2025. https://doi.org/10.1101/2025.07.14.664780

- benchmark pretrained DNA language model for TF binding site prediction.

**GeneCAD: Plant Genome Annotation with a DNA Foundation Model.**

Zong-Yan Liu,  ProfileAna Berthel,  ProfileEric Czech,  Michelle Stitzer,  Sheng-Kai Hsu, Matt Pennell,  Edward S. Buckler,  Jingjing Zhai. *bioRxiv*, Oct 2025. https://doi.org/10.1101/2025.10.31.685877

**Semantic design of functional de novo genes from a genomic language model.** 

Merchant, A.T., King, S.H., Nguyen, E. *et al.* *Nature* **649**, 749–758 (2026). https://doi.org/10.1038/s41586-025-09749-7


## CRE engineering and design

### Traditional

**Synthetic promoter designs enabled by a comprehensive analysis of plant core promoters.**

Jores, T., Tonnies, J., Wrightsman, T. *et al.*  *Nat. Plants* **7**, 842–855 (2021). https://doi.org/10.1038/s41477-021-00932-y



### Supervised

**Deciphering the sequence basis and application of transcriptional initiation regulation in plant genomes through deep learning.**

Gao, P., Lian, L., Feng, W. *et al.*  *Genome Biol* **26**, 293 (2025). https://doi.org/10.1186/s13059-025-03782-5

- GenoRetriever is designed to accurately predict TSS locations and transcriptional abundance while elucidating sequence contributions to transcription initiation.

**Rational design of promoter editing confers multipathogen resistance in rice.** 

Han, X., Yang, L., Xia, F. *et al.* *Genome Biol* **26**, 371 (2025). https://doi.org/10.1186/s13059-025-03841-x

- use PlantDeepSEA for mutagenesis analysis of RBL1 promoter (screening the mutations that increase chromatin accessibility)

**PlantDeepSEA, a deep learning-based web service to predict the regulatory effects of genomic variants in plants.** 

Zhao, H., Tu, Z., Liu, Y., Zong, Z., Li, J., Liu, H., Xiong, F., Zhan, J., Hu, X., & Xie, W. (2021). *Nucleic acids research*, *49*(W1), W523–W529. https://doi.org/10.1093/nar/gkab383



#### other species

**Designing synthetic regulatory elements using the generative AI framework DNA-Diffusion。**

DaSilva, L.F., Senan, S., Kribelbauer-Swietek, J.F. *et al.* *Nat Genet* **58**, 180–194 (2026). https://doi.org/10.1038/s41588-025-02441-6

**Massively parallel characterization of transcriptional regulatory elements.** 

Agarwal, V., Inoue, F., Schubach, M. *et al.* *Nature* **639**, 411–420 (2025). https://doi.org/10.1038/s41586-024-08430-9

- This study combines MPRA data with machine learning to model *cis*-regulatory elements, and offers an extensive catalogue of functional CREs in three widely used cell lines and showcases how large-scale functional measurements can be used to dissect regulatory grammar.

**Machine-guided design of cell-type-targeting *cis*-regulatory elements.**

Gosai, S.J., Castro, R.I., Fuentes, N. *et al.* Machine-guided design of cell-type-targeting *cis*-regulatory elements. *Nature* **634**, 1211–1220 (2024). https://doi.org/10.1038/s41586-024-08070-z

- This study demonstrated convolutional neural network (CNN)-based optimization for regulatory element design, achieving high signal intensity through direct optimization.

**Cell-type-directed design of synthetic enhancers.**

Taskiran, I.I., Spanier, K.I., Dickmänken, H. *et al.* *Nature* **626**, 212–220 (2024). https://doi.org/10.1038/s41586-023-06936-2

**Predicting effects of noncoding variants with deep learning–based sequence model.** 

Zhou, J., Troyanskaya, O. *Nat Methods* **12**, 931–934 (2015). https://doi.org/10.1038/nmeth.3547

### Unsupervised





## protein engineering and design

**Large DNA and protein language models enhance discovery of deleterious mutations in maize.**

Wang, Y., He, C., Wang, Q. *et al.* *Genome Biol* **26**, 412 (2025). https://doi.org/10.1186/s13059-025-03890-2



### other species



## Phylogenetics

### Software

**Fast and accurate protein structure search with Foldseek.**

van Kempen, M., Kim, S.S., Tumescheit, C. et al.  *Nat Biotechnol* 42, 243–246 (2024). https://doi.org/10.1038/s41587-023-01773-0

**Multiple protein structure alignment at scale with FoldMason.**

Gilchrist, C. L. M., Mirdita, M., & Steinegger, M. *Science* 391(6784):485-488. http://doi.org/10.1126/science.ads6733


### Others
**Structural genomics sheds light on protein functions and remote homologs across the insect tree of life.**

Wu, W., Cui, C., Zhu, Y. *et al.* *Cell Res* (2026). https://doi.org/10.1038/s41422-026-01220-0



## Others

**PlantScience.ai: An LLM-Powered Virtual Scientist for Plant Science**

Haopeng Yu, Shasha Zhou, Mingyu Huang, Ling Ding, Yuxuan Chen, Yinru Wang, Yingyu Ren, Nuo Cheng, Xinya Wang, Jie Liang, The John Innes Centre and The Sainsbury Laboratory Collaboration, Huakun Zhang, Yiliang Ding, Ke Li. *bioRxiv, October 2025* https://doi.org/10.1101/2025.10.24.684337

## Dataset

### Genome & Variation

- RiceVarMap – rice genomic variation database

- Genome Variation Map (GVM) – variation across 29 plants

- Plant ImputeDB – genotype imputation database

- CropGS-Hub – genomic selection & GWAS signals

- GWAS Atlas – GWAS results across 15 specie

### Gene Regulation
#### TF and CRE

- PlantRegMap

- PlantPAN 4.0

- PlantTFDB

- AGRIS

- GRASSIUS

- JASPAR

- UniBind

- GTRD

- ChIPBase

- PCSD

- PODC

- Rice Regulome Atlas

#### DNA Methylation
- MethBank

- ASMdb

- PRP

- PlantEpigenome (13 plants)

- m6A-Atlas v2.0

#### Chromatin Accessibility & Histone
- PlantCADB – ATAC-seq for 37 species

- ChIP-Hub – ChIP / DAP / ATAC / DNase

- RiceENCODE

- AraENCODE

- WERAM

### Transcriptomics
- PlantExp – 131k RNA-seq from 85 plants

- JGI Plant Gene Atlas – >2000 RNA-seq samples

- GERDH – RNA-seq / ChIP-seq / m6A / DNAme

- PEO – plant expression atlas

- Expression Atlas

- Stress Knowledge Map

### Multi-omics / Regulomics

- Plant Regulomics – 6 plants

- PPGR – Plant Public Genomic Resources

- Plant MAP – plant protein complex & PPI

- wGRN – gene regulatory network

- MaizeNetome

### Protein & PTM

- qPTMplants – 23 PTM types, 43 plants

- P3DB – phosphorylation database

- MPOD – multi-omics proteomics resource

### Non-coding RNA

- PlantcircBase – circRNA database

- JustRNA – plant lncRNA

- uORFlight – plant uORF database

## Single-cell & Spatial omics

- PlantscRNAdb – scRNA-seq for 8 species

- PsctH – Plant Single Cell Transcriptome Hub

- PCMDB – plant cell marker database

- scAPAdb – single-cell APA database

- RCAR – rice root single-cell atlas

- PlantPhoneDB – cell–cell communication

- SPEED – pan-species single-cell profiles

- SODB – plant spatial omics database

## Gene Family / Evolution

- CropGF – crop gene family database

- Homologous Gene Database (HGD)

## Species-specific Databases
### Rice

- RiceENCODE

- Rice Regulome Atlas

### Maize

- MaizeGDB

- ZEAMAP

### Soybean

- SoyOmics

- SoyMD

### Cotton

- CottonMD

### Brassica

- BRAD

- BnaOmics

- BnIR

### Other crops

- LettuceGDB

- Milletdb

- ArecaceaeMDB

- Malvaceae database

- TCMPG

## Gene Editing

- EU-SAGE – plant gene editing resources

- CRISPRbase – CRISPR editing database

## Other Resources

- PhyloPic – plant/animal illustrations

- AGBioData – agricultural biological databases

## Others



⭐ If you find this repository useful, please consider starring it.

📢 Contributions are welcome! Please read [CONTRIBUTING.md](CONTRIBUTING.md) before suggesting new papers or suggestions.